...
def plot_insight(df, graph_name, x_label, y_label):
    plt.plot(df[x_label], df[y_label], label="WOW")
    plt.xlabel(x_label)
    plt.ylabel(y_label)
...
def mlib_data(df, country):
    df = df.filter("Variable == 'TMAX' or Variable == 'TMIN' or Variable == 'SNOW' or Variable =='PRCP'").withColumn("month", F.month("fullDate"))
...
def main():
    ...
    # # df = read_df_from_sql_server('AS_PRCP')
    # # df = df.withColumn('weighted_sum', F.col('PRCP')*F.col('PRCP_count')).alias('Sum').drop(F.col('PRCP'))
    # # df = df.groupBy(F.col('elevation')).agg(F.sum(F.col('weighted_sum')), F.sum(F.col('PRCP_count')))
    # # df = df.withColumn('avg',F.col('sum(weighted_sum)')/F.col('sum(PRCP_count)')).drop(F.col('sum(weighted_sum)')).drop(F.col('sum(PRCP_count)'))
    # # #removing outliers
    # # df = df.filter(F.col('elevation') < 2000).filter(F.col('avg') < 120)
    # # panda_df = df.toPandas().sort_values(by='elevation')
    # # plot_insight(panda_df,graph_name='US_PRCP',x_label='elevation',y_label='avg')
    #
    # df = read_df_from_sql_server('AS_SNOW')
    # df = df.withColumn('weighted_sum', F.col('SNOW') * F.col('SNOW_count')).alias('Sum').drop(F.col('SNOW'))
    # df = df.groupBy(F.col('elevation')).agg(F.sum(F.col('weighted_sum')), F.sum(F.col('SNOW_count')))
    # df = df.withColumn('avg', F.col('sum(weighted_sum)') / F.col('sum(SNOW_count)')).drop(
    #     F.col('sum(weighted_sum)')).drop(F.col('sum(Snow_count)'))
    # # removing outliers
    # #df = df.filter(F.col('elevation') < 2000).filter(F.col('avg') < 120)
    # panda_df = df.toPandas().sort_values(by='elevation')
    # plot_insight(panda_df, graph_name='US_SNOW', x_label='elevation', y_label='avg')
