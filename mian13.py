...
def readStreamData(country, var, amount, transformation_func):
    os.environ['PYSPARK_SUBMIT_ARGS'] = "--packages=org.apache.spark:spark-sql-kafka-0-10_2.12:3.1.1," \
                                        "com.microsoft.azure:spark-mssql-connector_2.12:1.1.0 pyspark-shell"
    spark, sc = init_spark('RoiAndYarin')
    kafka_server = 'dds2020s-kafka.eastus.cloudapp.azure.com:9092'
    ...
    query = streaming_input_df \
        .writeStream \
        .outputMode("append") \
        .foreachBatch(lambda df, epoch_id: transformation_func(df, country, var)) \
...
def spatial_info(df, country, var):
    ...
    write_to_server(df_joineds, country + '_' + var)


def us_precipitation(df, country, var):
    df_stations = read_df_from_sql_server('GHCND_Stations')
    df_stations = df_stations.select(['StationID', 'ELEVATION'])
    ...
    df = df.join(df_stations, on='StationID').drop('Variable', 'StationId').withColumnRenamed('Value', var).filter(F.col('ELEVATION')!=-999.9)
    df = df.drop('fullDate', 'Month')
    df = df.groupBy(['ELEVATION']).agg(F.avg(var).alias(var)).orderBy(F.col('ELEVATION').asc())
...
def plot_insight(df, graph_name, x_label, y_label):
    df = df.toPandas()
    plt.plot(df['elevation'], df['SNOW'], label="WOW")
    plt.xlabel(x_label)
    plt.ylabel(y_label)
...
def fetch_data_from_kafka():
    ...
    #     for var in var_lst:
    #         print(f"Current time passed: {time.time()-initial_time}")
    #         readStreamData(country, var, amount)
    readStreamData('US', 'PRCP', amount, us_precipitation)
